...
public class Reset extends Controller {
	...
	public static Result ask() {
		...
		return ok(views.html.account.reset.ask.render(askForm));
	}
	...
	public static Result runAsk() {
		...
		if (askForm.hasErrors()) {
			...
			return badRequest(views.html.account.reset.ask.render(askForm));
		}
	...
	public static Result runReset(String token) {
		...
		if (resetForm.hasErrors()) {
			...
			return badRequest(views.html.account.reset.reset.render(resetForm,
					token));
		}
		...
		try {
			...
			if (resetToken == null) {
				flash("error", Messages.get("error.technical"));
				return badRequest(views.html.account.reset.reset.render(
						resetForm, token));
			}
