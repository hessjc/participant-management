...
public class MyDynamicResourceHandler implements DynamicResourceHandler {
	...
	static {
		...
		HANDLERS.put("viewProfile", new AbstractDynamicResourceHandler() {
			public boolean isAllowed(final String name, final String meta,
						...
						.map(new F.Function<Subject, Boolean>() {
							...
							public Boolean apply(Subject subject)
								...
								if (DeadboltAnalyzer.hasRole(subject, "admin")) {
									...
									// a call to view profile is probably a get
									// request, so
									...
									allowed = requestedNames != null
											...
											&& requestedNames[0].equals(subject
													.getIdentifier());
								}
		...
		return deadboltHandler.getSubject(ctx)
				.map(new F.Function<Subject, Boolean>() {
					...
					public Boolean apply(Subject subject) throws Throwable {
						...
						if (subject != null) {
							List<? extends Permission> permissions = subject
									.getPermissions();
							for (Iterator<? extends Permission> iterator = permissions
									.iterator(); !permissionOk
									&& iterator.hasNext();) {
								Permission permission = iterator.next();
								permissionOk = permission.getValue().contains(
										permissionValue);
							}
						}
